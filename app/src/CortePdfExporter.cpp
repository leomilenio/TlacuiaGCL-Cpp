...
<table style="width:100%; margin-top:40px; border-collapse:collapse;" cellpadding="0" cellspacing="0">
  <tr>
    ...
    const QString watermark = preview ? QStringLiteral("PRE-CORTE") : QString();
    App::Pdf::renderPages(doc, printer, footerHPx, lineGapPx, contentPx, docSize,
                          folio, fecha, contact, watermark);
    return true;
}

bool CortePdfExporter::exportarProveedor(const Calculadora::ConcesionRecord&       concesion,
                                         const QList<Calculadora::ProductoRecord>& productos,
                                         const Calculadora::CorteResult&           corte,
                                         const Calculadora::LibreriaConfig&        config,
                                         const QString&                            folioDocumento,
                                         ...
                                         bool                                      preview)
{
    ...
    const QString cssPropio = R"(
  table.prod    { width:100%; margin-top:4px; }
  table.prod th { background:#2c5f8a; color:white; padding:5px 6px; font-size:9.5pt; }
  table.prod td { padding:4px 6px; font-size:9.5pt; border-bottom:1px solid #e0e0e0; }
  table.prod tr:nth-child(even) td { background:#f5f5f5; }
  .tot-lbl      { text-align:right; color:#555; padding:3px 8px; }
  .tot-val      { font-weight:bold; text-align:right; padding:3px 8px; }
  .iva-line     { border-top:1px solid #ccc; margin-top:6px; padding-top:6px; }
  hr.sep        { border:none; border-top:1px solid #ccc; margin:8px 0; }
)";
...
<hr class="sep">
    ...
    // ---- Tabla de productos (sin columna Rec.) ----
    html += R"(
<h2>Detalle de Productos</h2>
<table class="prod">
<thead><tr>
  ...
  <td style="text-align:right">%6</td>
             ...
             QString::number(vendida), QString::number(devuelta),
             fmt(p.costo), fmt(p.costo * vendida));
    }
    html += "</tbody></table>\n";
    ...
    html += QString(R"(
<table style="margin-left:auto; margin-top:10px;">
  <tr><td class="tot-lbl">Total a pagar:</td><td class="tot-val">%1</td></tr>
  <tr><td class="tot-lbl">Total piezas a devolver:</td><td class="tot-val">%2</td></tr>
</table>
)").arg(fmt(corte.totalPagoAlDistribuidor),
        QString::number(corte.totalUnidadesDevueltas));

    // ---- IVA proveedor (solo si emite CFDI) ----
    if (concesion.emisorFacturacion) {
        html += QString(R"(
<div class="iva-line">
<table style="margin-left:auto;">
  <tr><td class="tot-lbl">IVA sobre ventas (según CFDI):</td><td class="tot-val">%1</td></tr>
</table>
</div>
)").arg(fmt(corte.totalIvaAcreditable));
    }
